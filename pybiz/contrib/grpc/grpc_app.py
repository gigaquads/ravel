...
class Grpc(Application):
    ...
    def on_request(self, endpoint, request, *args, **kwargs):
        """
        Take the attributes on the incoming protobuf Message object and
        map them to the args and kwargs expected by the endpoint target.
        """
        print(f'>>> Calling "{endpoint.name}" RPC function...')

        # get field data to process into args and kwargs
        all_arguments = {
            k: getattr(request, k, None)
            for k in endpoint.request_schema.fields
        }

        # process field_data into args and kwargs
        args, kwargs = FuncUtils.partition_arguments(
            endpoint.signature, all_arguments
        )
        return (args, kwargs)

    def on_response(self, endpoint, result, *raw_args, **raw_kwargs):
        """
    ...
    def _grpc_servicer_factory(self):
        ...
        module and derive a new subclass that inherits this Application's endpoint
    ...
    def _grpc_generate_proto_file(self):
        """
        Iterate over function endpoints, using request and response schemas to
